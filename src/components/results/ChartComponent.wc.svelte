...
<script lang="ts">
            ...
            chartType === "bar"
              ? {
                  callback: (val: string | number) => {
                    ...
                    const key: unknown =
                      initialChartData.data.labels[val] !== undefined
  ...
  const combineSubGroups = (
    ...
    for (const label of labels) {
      ...
      if (!label.includes(divider) || divider === "") {
        ...
        labelsToData.set(superClassLabel, oldValue ? oldValue + value : value);
      }
    }
  ...
  const setChartData = (responseStore: ResponseStore): void => {
     ...
     */
    let chartData: ChartDataSets = getChartDataSets(responseStore, chartLabels);
    ...
    chart.data.labels =
      options?.legendMapping !== undefined
        ? chartLabels.map((label) => options.legendMapping?.[label] || "")
        : chartLabels;
          ...
          if (
            ...
            if (childCategorie.fieldType === "number") {
              ...
              childCategorie.criteria.forEach((criterion: Criteria) => {
                if (criterion.key === label || criterion.name === label) {
                  values[0] = {
                    name: criterion.name,
                    value: criterion.key,
                    queryBindId: uuidv4(),
