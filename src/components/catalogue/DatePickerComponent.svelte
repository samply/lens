<script lang="ts">
  ...
  const handleInputFrom = (): void => {
    if (from === null || from === "" || from === undefined) {
      from = (element.min as string) || new Date().toISOString().split("T")[0];
    }
  };

  const handleInputTo = (): void => {
    if (to === null || to === "" || to === undefined) {
      to = (element.max as string) || new Date().toISOString().split("T")[0];
    }
  };
</script>
  ...
  <div part="criterion-item">
    <div part="criterion-section criterion-section-values">
      ...
      >
        ...
        <input
          part="date-input-formfield date-input-formfield-from {to && from > to
            ? ' formfield-error'
            : ''}"
          type="date"
          bind:value={from}
      ...
      >
        ...
        <input
          part="date-input-formfield date-input-formfield-to{to && from > to
            ? ' formfield-error'
            : ''}"
          type="date"
          bind:value={to}
