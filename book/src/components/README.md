# Components

Here we list all the Lens components:

- [CatalogueComponent](catalogue.md)
- [ChartComponent](chart.md)
- [InfoButtonComponent](infobutton.md)
- [modifiedsearchComponent](modifiedsearch.md)
- [NegotiateButtonComponent](negotiatebutton.md)
- [QueryExplainButtonComponent](queryexplainbutton.md)
- [QuerySpinnerComponent](queryspinner.md)
- [ResultsummeraryComponent](resultsummary.md)
- [ResultTableComponent](resulttable.md)
- [SearchbarComponent](searchbar.md)
- [SearchbarComponentMultiple](searchbarmulti.md)
- [SearchButtonComponent](searchbutton.md)
- [StoreDeleteButtonComponent](storedeletebutton.md)
