...
| `clickToAddState`      | `boolean`             | `"false"`  | Set to true to add the clicked bar or pie slice to the search. `dataKey` must be the key of a `single-select`, `autocomplete` or `number` item, and the stratum code behind the element must match a criterion `key`, or else a `name`. No match, no change. Ignored when `perSite` is set.       |
...
| `enableSorting`        | `boolean`             | `"false"`  | Enables sorting buttons for chart                                                                                                                                                                                                                                                                 |
...
| `topN`                 | `number`              | `""`       | Limits the chart to the top N data entries sorted by value in descending order. Useful for e.g. showing the top 20 modified genes.                                                                                                                                                                |
...
| `hoverBackgroundColor` | `string[]`            | `""`       | Similar functionality as the background color, instead the hover colors.                                                                                                                                                                                                                          |
