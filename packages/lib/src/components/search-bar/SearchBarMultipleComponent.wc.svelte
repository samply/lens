...
<script lang="ts">
    ...
    import SearchButtonComponenet from "../buttons/SearchButtonComponenet.wc.svelte";
    ...
    export let button: any = { buttonActive: true, title: "Search Biobanks" };

    const addSearchBar = () => {
        queryStore.update((queryStore) => {
            queryStore.push([]);
            return queryStore;
        });
    };
...
<div part="lens-searchbar-multiple">
    {#each $queryStore as queryGroup, index}
        <div part="search-bar-wrapper">
            ...
            <button part="lens-searchbar-add-button" on:click={addSearchBar}>+</button>
            {:else}
        ...
        {#if button.buttonActive}
            <SearchButtonComponenet title={button.title} />
        {/if}
</div>
