<script lang="ts">
    ...
    import { addItemToQuery, removeItemFromQuery } from "../../stores/query";
...
<label part="lens-catalogue-query-select-label">
    {index + 1}
    <input part="lens-catalogue-query-select-input" type="checkbox" on:change={e => toggleSelectedGroup(!isChecked)} checked={isChecked}>
</label>