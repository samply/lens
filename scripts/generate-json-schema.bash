if [ "$1" = "--check" ]; then
    ...
    npx ts-json-schema-generator --path src/types/catalogue.ts --type CatalogueInput | cmp - schema/catalogue.schema.json
    npx ts-json-schema-generator --path src/types/options.ts --type LensOptions | cmp - schema/options.schema.json
else
    npx ts-json-schema-generator --path src/types/catalogue.ts --type CatalogueInput > schema/catalogue.schema.json
