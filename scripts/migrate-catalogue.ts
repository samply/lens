...
type AstNode =
    | { type: "AndOperator"; operands: AstNode[] }
    | { type: "OrOperator"; operands: AstNode[] }
...
function convertAggregatedValue(
    ...
    const andGroups: AstNode[] = agg.map((group) => {
        const filters: AstNode[] = group.map((item) => ({
            type: "SetFilter" as const,
            key: item.value,
            values: [item.name],
        }));
        if (filters.length === 1) return filters[0];
        return { type: "AndOperator" as const, operands: filters };
    });

    if (andGroups.length === 1) return andGroups[0];
    return { type: "OrOperator" as const, operands: andGroups };
}
...
function convertCriteria(criterion: OldCriteria): NewCatalogueOption {
    ...
    if (criterion.visible !== undefined) {
        option.visible = criterion.visible;
    }

    if (criterion.aggregatedValue) {
        const ast = convertAggregatedValue(criterion.aggregatedValue);
        if (ast) {
            option.ast = ast;
        } else {
            console.warn(
